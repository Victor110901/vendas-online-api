...
describe('CartProductService', () => {
  ...
  it('should return error in exception delete', async () => {
    ...
    expect(service.deleteProductCart(
      productMock.id, 
      cartMock.id
  ...
  it('should return error in exception save', async () => {
    ...
    expect(service.createCartProductInCart(
      insertCartMock, 
      cartMock.id
    )).rejects.toThrowError()
  });
  ...
  it('should return error if not found', async () => {
    ...
    expect(service.verifyProductInCart(
      productMock.id, 
      cartMock.id
  ...
  it('should return error in insertProductInCart', async () => {
    jest
      .spyOn(productService, 'findProductById')
      .mockRejectedValue(new NotFoundException())

    expect(service.insertProductInCart(
      insertCartMock, 
      cartMock
    )).rejects.toThrowError(NotFoundException)
  });
  ...
  it('should return error in updateProductInCart', async () => {
    jest
      .spyOn(productService, 'findProductById')
      .mockRejectedValue(new NotFoundException())
  ...
  it('should return cartProduct if not exist cart (updateProductInCart)', async () => {
    jest.spyOn(cartProductRepository, 'findOne').mockResolvedValue(undefined)
